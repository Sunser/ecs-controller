...
func renderGlobalSettings(original string, cfg Config) string {
	...
	writeKeyValue(&builder, "traffic_exceeded_notify_interval", quote(formatDuration(cfg.Notification.TrafficExceededNotifyInterval.String())))
	...
	writeKeyValue(&builder, "operation_cooldown", quote(formatDuration(cfg.KeepAlive.OperationCooldown.String())))
